...
class RobotTeamServer {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                if (axios.isAxiosError(error)) {
                    ...
                    throw new McpError(status === 401 ? ErrorCode.InvalidRequest : ErrorCode.InternalError, `Robot Team API error: ${message}`);
                }
                throw new McpError(ErrorCode.InternalError, `Unexpected error: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    async sendRobotMessage(args) {
        ...
        const robotInfo = ROBOT_COLORS[robotId];
        return {
            content: [{
                    type: 'text',
    ...
    async scheduleRobotMessage(args) {
    ...
    async getRobotStatus(args) {
        ...
        if (robotId) {
            ...
            robots.forEach(robot => {
                const robotInfo = ROBOT_COLORS[robot.id];
    ...
    async listScheduledMessages(args) {
        ...
        const params = { limit, offset, active };
        if (robotId)
        ...
        schedules.forEach((schedule, index) => {
            const robotInfo = ROBOT_COLORS[schedule.robotId];
    ...
    async cancelScheduledMessage(args) {
    ...
    async generateApiKey(args) {
        if (!ADMIN_KEY) {
            throw new McpError(ErrorCode.InvalidRequest, 'Admin key not configured. Set ROBOT_TEAM_ADMIN_KEY environment variable.');
        }
